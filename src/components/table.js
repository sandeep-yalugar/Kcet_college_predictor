...
const Table = () => {
    ...
    const handleNext = () => 
    {
       const maxind = ldis.length;
        if((tbase+10)<maxind){tbase = tbase + 10;}
        if ((crind+tbase+10)<ldis.length)
        {
          const payload = ldis.slice(crind+base,crind+tbase+10).map((obj,ind) => {return {...obj,sino:ind+tbase+1}});
          setlis(payload);
        }
          else
          {
            const payload = ldis.slice(crind+tbase,ldis.length).map((obj,ind) => {return {...obj,sino:ind+tbase+1}});
          setlis(payload);
          }
    }
